...
This guide explains how to configure your system's firewall to allow incoming connections to Drive services. These instructions are for your local host system where Drive is running.
...
When you run Drive services, they need to accept incoming connections on specific ports. Your system's firewall may block these connections by default. This guide shows you how to configure your firewall to allow these connections.

**What ports need to be opened?**
- Each service has specific ports that need to be accessible
- See the [Service Catalog]({{< relref "../catalog" >}}) for port configurations for each service
...
**Important:** If UFW is already active, you don't need to enable it again. Just make sure SSH is allowed.
...
### Method 1: Windows Defender Firewall (Graphical Interface)
...
```powershell
New-NetFirewallRule -DisplayName "Drive Service Ports" -Direction Inbound -LocalPort 26656,26657,9090 -Protocol TCP -Action Allow
```
...
### Port Still Blocked After Configuration
...
For service-specific firewall configurations with actual port numbers, see the individual service documentation in the [Service Catalog]({{< relref "../catalog" >}}).
...
- [Service Catalog]({{< relref "../catalog" >}}) - Service-specific port configurations
